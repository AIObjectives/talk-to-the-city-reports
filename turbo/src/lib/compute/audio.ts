	...
	async compute(
		...
		if (this.data.gcs_path) {
			if (this.data.download) {
				...
				this.data.output = new File([blob], this.data.filename, { type: this.data.mime_type });
			}
		}
...
export let audio_node_data: AudioNodeInterface = {
	id: 'audio',
	data: {
		label: 'audio',
		filename: '',
...
export let audio_node = new AudioNode(audio_node_data);
