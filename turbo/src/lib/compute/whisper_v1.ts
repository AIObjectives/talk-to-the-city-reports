...
export default class WhisperNode {
  ...
  convert_to_internal_format(doc: any, id: string) {
    const output: any[] = [];
  ...
  async compute(
    ...
    if (!this.data.dirty && this.data.gcs_path && sizes_match) {
      ...
      if (_.includes(['verbose_json', 'json'], this.data.response_format)) {
        ...
        this.data.output = this.convert_to_internal_format(JSON.parse(doc), this.id);
        return this.data.output;
      }
      return this.data.output;
    }
    ...
    if (context == 'run' && audio && open_ai_key) {
      ...
      if (this.data.response_format == 'custom') {
        this.data.output = this.convert_to_internal_format(transcription, this.id);
        return this.data.output;
      }
