import StringifyNode, { stringify_node_data } from '$lib/compute/stringify';
...
describe('StringifyNode class', () => {
	...
	it('should handle different types of input', async () => {
		...
		const resultArray = await node.compute(inputDataArray, 'run', console.log, console.error, console.log, '/');
		expect(resultArray).toBe(JSON.stringify(inputDataArray.input, null, 2));

		const inputDataObject = { input: { a: 1, b: 2 } };
		const resultObject = await node.compute(inputDataObject, 'run', console.log, console.error, console.log, '/');
		expect(resultObject).toBe(JSON.stringify(inputDataObject.input, null, 2));

		const inputDataString = { input: 'test string' };
		const resultString = await node.compute(inputDataString, 'run', console.log, console.error, console.log, '/');
